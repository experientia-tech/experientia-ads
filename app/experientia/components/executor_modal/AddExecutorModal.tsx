...
import styles from './AddExecutorModal.module.scss';
...
const AddExecutorModal: React.FC<AddExecutorModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  organizationId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create executor');
      }

      const newExecutor = await response.json();
      onSuccess(newExecutor.data || newExecutor);
      onClose();
      ...
      console.error('Error creating executor:', err);
      setError(err instanceof Error ? err.message : 'Failed to create executor');
    } finally {
      setIsLoading(false);
    }
  };
