...
const totalCampaigns = brands.reduce((sum, brand) => sum + brand.totalCampaigns, 0);
const activeCampaigns = brands.reduce((sum, brand) => sum + brand.activeCampaigns, 0);
const totalTasks = brands.reduce((sum, brand) => 
  sum + brand.campaigns.reduce((taskSum, campaign) => taskSum + campaign.totalTasks, 0), 0);
const completedTasks = brands.reduce((sum, brand) => 
  sum + brand.campaigns.reduce((taskSum, campaign) => taskSum + campaign.completedTasks, 0), 0);
const pendingTasks = totalTasks - completedTasks;
const flaggedTasks = brands.reduce((sum, brand) => 
  sum + brand.campaigns.reduce((taskSum, campaign) => taskSum + campaign.flaggedTasks, 0), 0);
      ...
      <div className={styles.brandsSection}>
        {brands.map((brand) => (
          <div key={brand.id} className={styles.brandContainer}>
            ...
            />
            <div className={`${styles.campaignsList} ${expandedBrandId === brand.id ? styles.expanded : styles.collapsed}`}>
